    ...
    public partial class Form_HoaDonBan : Form
    {
        ...
        void LoadHoaDonBan()
        {
            try
            {
                ...
                dgvDanhSachHoaDonBan.Columns[3].HeaderText = "Mã khách hàng";
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (Them)
            {
                try
                {
                    kq = hdbbusiness.ThemHoaDonBan(ref err, DateTime.Parse(dtpNgayBan.Text), int.Parse(txtMaNhanVien.Text),int.Parse(txtMaKH.Text));
                    if (kq)
                    {
            ...
            else
            {
                ...
                dgvDanhSachHoaDonBan.Rows[r].Cells[0].Value.ToString();
                // Câu lệnh SQL 
                kq = hdbbusiness.CapNhatHoaDonBan(ref err, int.Parse(txtMaHoaDonBan.Text), DateTime.Parse(dtpNgayBan.Text), int.Parse(txtMaNhanVien.Text), int.Parse(txtMaKH.Text));
                if (kq)
                {
                    // Load lại dữ liệu trên DataGridView 
                    LoadHoaDonBan();
        ...
        private void dgvDanhSachHoaDonBan_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            this.dtpNgayBan.Text = dgvDanhSachHoaDonBan.Rows[r].Cells[1].Value.ToString();
            this.txtMaNhanVien.Text = dgvDanhSachHoaDonBan.Rows[r].Cells[2].Value.ToString();
            this.txtMaKH.Text = dgvDanhSachHoaDonBan.Rows[r].Cells[3].Value.ToString();
        }
    }
}
