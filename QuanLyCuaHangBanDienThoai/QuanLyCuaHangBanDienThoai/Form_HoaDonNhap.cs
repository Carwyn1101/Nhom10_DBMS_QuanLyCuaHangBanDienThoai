    ...
    public partial class Form_HoaDonNhap : Form
    {
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (Them)
            {
                try
                {
                    kq = hdnbusiness.ThemHoaDonNhap(ref err, dtpNgayNhap.Text, int.Parse(txtMaNhanVien.Text));
                    if (kq)
                    {
            ...
            else
            {
                ...
                dgvDanhSachHoaDonNhap.Rows[r].Cells[0].Value.ToString();
                // Câu lệnh SQL 
                kq = hdnbusiness.CapNhatHoaDonNhap(ref err, int.Parse(txtMaHoaDonNhap.Text), dtpNgayNhap.Text, int.Parse(txtMaNhanVien.Text));
                if (kq)
                {
                    // Load lại dữ liệu trên DataGridView 
                    LoadHoaDonNhap();
        ...
        private void dgvDanhSachHoaDonNhap_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            this.dtpNgayNhap.Text = dgvDanhSachHoaDonNhap.Rows[r].Cells[1].Value.ToString();
